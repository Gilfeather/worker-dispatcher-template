...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    dispatcher_service = WorkerDispatcherImpl(
        task_repository, worker_repository, result_repository, worker_management_service
    )
...
def get_dispatcher_service() -> WorkerDispatcherImpl:
    if dispatcher_service is None:
        raise HTTPException(
            status_code=500, detail="Dispatcher service not initialized"
        )
...
def get_worker_management_service() -> WorkerManagementImpl:
    if worker_management_service is None:
        raise HTTPException(
            status_code=500, detail="Worker management service not initialized"
        )
