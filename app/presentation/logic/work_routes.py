...
from ..interface import get_dispatcher_service, get_worker_management_service, get_result_repository
from .schemas import (
    WorkerCreateRequest, 
    WorkerResponse, 
...
async def complete_task(
    ...
    try:
        ...
        if completion.execution_time_ms is not None:
            ...
            if result:
                result.execution_time = timedelta(milliseconds=completion.execution_time_ms)
...
async def process_tasks_continuously(
    ...
    try:
        ...
        import logging
        logging.error(f"Error in continuous task processing for worker {worker_id}: {e}")
