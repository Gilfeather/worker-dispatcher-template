...
async def get_metrics(
    ...
    return {
        ...
        "tasks": task_stats,
        "system": {"uptime": "calculated from startup", "version": "1.0.0"},
    }
...
async def mark_unhealthy_workers_offline(
    ...
    try:
        offline_ids = await worker_management.mark_unhealthy_workers_offline(
            timeout_seconds
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Maintenance operation failed: {str(e)}"
        )
