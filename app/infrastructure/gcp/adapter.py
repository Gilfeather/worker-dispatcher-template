...
class GCPStorageAdapter:
    ...
    async def upload_blob(
        ...
        try:
            ...
            await loop.run_in_executor(
                self.executor, blob.upload_from_filename, source_file_path
            )
    ...
    async def download_blob(
        ...
        try:
            ...
            await loop.run_in_executor(
                self.executor, blob.download_to_filename, destination_file_path
            )
        except GoogleCloudError as e:
    ...
    async def list_blobs(self, bucket_name: str, prefix: str = "") -> List[str]:
        try:
            ...
            blobs = await loop.run_in_executor(
                self.executor, self.client.list_blobs, bucket_name, prefix
            )
...
class GCPBigQueryAdapter:
    ...
    async def query(self, query: str) -> List[Dict[str, Any]]:
        try:
            ...
            query_job = await loop.run_in_executor(
                self.executor, self.client.query, query
            )
            results = await loop.run_in_executor(self.executor, query_job.result)
    ...
    async def insert_rows(self, table_id: str, rows: List[Dict[str, Any]]) -> None:
        try:
            ...
            errors = await loop.run_in_executor(
                self.executor, self.client.insert_rows_json, table, rows
            )
            if errors:
    ...
    async def create_table(
        ...
        try:
            ...
            await loop.run_in_executor(self.executor, self.client.create_table, table)
        except GoogleCloudError as e:
