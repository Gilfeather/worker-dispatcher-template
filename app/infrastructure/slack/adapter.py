...
class SlackAdapter:
    ...
    async def send_task_notification(
        self, task_name: str, status: str, worker_name: str, channel: str
    ) -> None:
    ...
    async def send_worker_alert(
        self, worker_name: str, alert_type: str, message: str, channel: str
    ) -> None:
    ...
    async def create_channel(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                payload = {"name": channel_name, "is_private": is_private}
                ...
                async with session.post(
                    ...
                    if not result.get("ok"):
                        raise SlackServiceException(
                            f"Failed to create channel: {result.get('error', 'Unknown error')}"
                        )
                    return result
        except aiohttp.ClientError as e:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if not result.get("ok"):
                        raise SlackServiceException(
                            f"Failed to get channel info: {result.get('error', 'Unknown error')}"
                        )
                    return result
        except aiohttp.ClientError as e:
    ...
    async def upload_file(
        self, file_path: str, channels: List[str], title: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                with open(file_path, "rb") as file:
                    ...
                    async with session.post(
                        f"{self.base_url}/files.upload", headers=headers, data=data
                    ) as response:
                        result = await response.json()
                        if not result.get("ok"):
                            raise SlackServiceException(
