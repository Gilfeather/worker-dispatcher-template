...
class S3Adapter:
    def __init__(self, region_name: str = "us-east-1", executor: Optional[ThreadPoolExecutor] = None):
    ...
    async def download_file(self, bucket_name: str, object_key: str, file_path: str) -> None:
        try:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(
                self.executor,
    ...
    async def object_exists(self, bucket_name: str, object_key: str) -> bool:
        try:
            ...
            if e.response['Error']['Code'] == '404':
                return False
            raise S3ServiceException(f"Failed to check object existence in S3: {str(e)}")
    
    async def generate_presigned_url(self, bucket_name: str, object_key: str, expiration: int = 3600) -> str:
        try:
            loop = asyncio.get_event_loop()
