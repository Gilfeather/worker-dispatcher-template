...
class WorkerManagementImpl(WorkerManagementService):
    ...
    async def get_worker_by_capabilities(self, capabilities: List[str]) -> List[Worker]:
        ...
        for worker in all_workers:
            if worker.is_available() and all(
                cap in worker.capabilities for cap in capabilities
            ):
    ...
    async def cleanup_offline_workers(
        ...
        for worker in all_workers:
            if (
                worker.status == WorkerStatus.OFFLINE
                and worker.last_heartbeat < cutoff_time
            ):
