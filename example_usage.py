...
class WorkerDispatcherClient:
    ...
    async def dispatch_task(self, name: str, payload: dict, priority: str = "medium"):
        ...
        async with self.session.post(f"{self.base_url}/dispatch/task", json=data) as response:
            return await response.json()
    
    async def dispatch_bulk_tasks(self, tasks: list):
        """Dispatch multiple tasks"""
        async with self.session.post(f"{self.base_url}/dispatch/task/bulk", json=tasks) as response:
            return await response.json()
    
    async def get_queue_status(self):
        """Get current queue status"""
        async with self.session.get(f"{self.base_url}/dispatch/queue/status") as response:
            return await response.json()
    
    async def get_available_workers(self):
        """Get available workers"""
        async with self.session.get(f"{self.base_url}/dispatch/available-workers") as response:
            return await response.json()
    ...
    async def register_worker(self, name: str, capabilities: list):
        ...
        async with self.session.post(f"{self.base_url}/work/register", json=data) as response:
            return await response.json()
    ...
    async def get_next_task(self, worker_id: str, capabilities: str = None):
        ...
        async with self.session.get(
            f"{self.base_url}/work/next-task",
            params={"worker_id": worker_id, **params}
        ) as response:
            return await response.json()
    
    async def complete_task(self, task_id: str, worker_id: str, result_data: dict = None):
        ...
        async with self.session.post(f"{self.base_url}/work/complete-task", json=data) as response:
            return await response.json()
    ...
    async def fail_task(self, task_id: str, worker_id: str, error_message: str):
        ...
        async with self.session.post(f"{self.base_url}/work/fail-task", json=data) as response:
            return await response.json()
    ...
    async def send_heartbeat(self, worker_id: str, status: str = None):
        ...
        async with self.session.post(f"{self.base_url}/work/heartbeat", json=data) as response:
            return await response.json()
...
async def demo_worker_workflow():
    ...
    async with WorkerDispatcherClient() as client:
        ...
        worker_result = await client.register_worker(
            name="Demo Worker",
            capabilities=["general", "data-processing", "reporting"]
        )
...
async def demo_full_workflow():
    ...
    print("This demo shows how the same container can act as both dispatcher and worker")
