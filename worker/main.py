...
class WorkerClient:
    ...
    async def register_worker(self):
        ...
        try:
            payload = {"name": self.worker_name, "capabilities": self.capabilities}

            async with self.session.post(
                f"{self.api_base_url}/api/v1/workers/", json=payload
            ) as response:
    ...
    async def process_task(self, task: Dict[str, Any]):
        ...
        try:
            ...
            await self.complete_task(
                task_id,
                {"result": "success", "processed_at": datetime.now().isoformat()},
            )
    ...
    async def complete_task(self, task_id: str, result_data: Dict[str, Any]):
        ...
        try:
            ...
            async with self.session.post(
                f"{self.api_base_url}/api/v1/tasks/{task_id}/complete", json=payload
            ) as response:
                if response.status != 200:
    ...
    async def fail_task(self, task_id: str, error_message: str):
        ...
        try:
            ...
            async with self.session.post(
                f"{self.api_base_url}/api/v1/tasks/{task_id}/fail", json=payload
            ) as response:
                if response.status != 200:
